...
def test_filter_made_in_belarus(web_browser):
    ...
    with allure.step('Открываем Каталог'):
        driver.catalog_button.click()

    with allure.step('Выбираем раздел продукты '):
        driver.katalog_prodycty.click()

    with allure.step('Прокручиваем до фильтра "Сделано в Беларуси"'):
        driver.scroll_down()

    with allure.step(' Включаем фильтр "Сделано в Беларуси"'):
        toggle=driver.made_in_belarus_toogle
        toggle.click()
        time.sleep(2)
        aria_checked=driver.toggle_checkbox.get_attribute("aria-checked")
        assert aria_checked == 'true'

    with allure.step('Проверяем, что фильтр применен'):
        bel_product = driver.made_in_belarus_product.click()
        proizvoditel = driver.strana_product.get_text()
        assert proizvoditel == 'Беларусь'
